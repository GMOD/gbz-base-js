...
async function walkBack(db: GBZBase, pos: Pos) {
  ...
  for (;;) {
    ...
    if (!record || offset === undefined) {
      throw new Error(`No offset in ${predecessor} leads to ${current.node}:${current.offset}`)
    }
...
async function checkIdentities(file: string, sample: string | undefined, contig: string, start: number, end: number, context: number) {
  const db = await GBZBase.open(new LocalFile(path.join(dataDir, file)))
  const subgraph = await subgraphInInterval(db, { contig, ...(sample === undefined ? {} : { sample }) }, start, end, { context })
  await subgraph.identifyPaths()
  const alignments = subgraph.alignments()
  expect(alignments.length).toBeGreaterThan(0)
  for (const alignment of alignments) {
    expect(alignment.pathHandle).toBeDefined()
    const gbzPath = await db.getPath(alignment.pathHandle as number)
    expect(gbzPath).toBeDefined()
    const { start: seqStart, bpBefore } = await walkBack(db, alignment.start)
    expect(seqStart).toEqual(alignment.strand === '+' ? gbzPath?.fwStart : gbzPath?.revStart)
    const length = (await db.haplotypeLength(alignment.pathHandle as number)) as number
    const local = {
      start: (alignment.hapStart as number) - (gbzPath?.name.fragment as number),
      end: (alignment.hapEnd as number) - (gbzPath?.name.fragment as number),
    }
    expect(alignment.strand === '+' ? local.start : length - local.end).toBe(bpBefore)
    let pathLen = 0
    ...
    expect(alignment.cigar.startsWith('D') || /^\d+D/.test(alignment.cigar)).toBe(false)
  }
...
describe('haplotype identification', () => {
  it('names every fragment in a context-free window', async () => {
    const { alignments } = await checkIdentities('micb-kir3dl1.gbz.db', 'GRCh38', 'chr6', 31500000, 31501000, 0)
    const samples = new Set(alignments.map(a => `${a.name?.sample}#${a.name?.haplotype}`))
  ...
  it('names every haplotype walk with context', async () => {
    const { alignments, subgraph } = await checkIdentities('micb-kir3dl1.gbz.db', 'GRCh38', 'chr6', 31500000, 31501000, 100)
    expect(alignments.length).toBe(subgraph.pathCount - 1)
    const json = subgraph.toJSON(true, { names: 'resolved' })
    expect(json.paths.slice(1).every(p => !p.name.startsWith('unknown#'))).toBe(true)
    expect(json.paths[1]?.name).toMatch(/^[A-Z0-9]+#[12]#\S+\[\d+-\d+\]$/)
  })

  it('works on the other reference and chromosome', async () => {
    await checkIdentities('micb-kir3dl1.gbz.db', 'CHM13', 'chr6', 31352000, 31352500, 20)
    await checkIdentities('micb-kir3dl1.gbz.db', 'GRCh38', 'chr19', 54816500, 54822000, 0)
  })
