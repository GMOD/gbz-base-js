...
export async function subgraphAtOffset(db: GBZBase, query: PathQuery, offset: number, opts: QueryOptions = {}) {
...
export async function subgraphInInterval(db: GBZBase, query: PathQuery, start: number, end: number, opts: QueryOptions = {}) {
  const subgraph = new Subgraph(db)
  subgraph.limit = opts.limit
...
export async function subgraphAroundNodes(db: GBZBase, nodes: number[], opts: QueryOptions = {}) {
