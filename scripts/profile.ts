...
const db = await GBZBase.open(new LocalFile('/Users/colin/src/gbz-base-js/test/data/micb-kir3dl1.gbz.db'))
const subgraph = new Subgraph(db)
console.time('pathPosition')
const ref = await subgraph.pathPosition({ sample: 'GRCh38', contig: 'chr19', haplotype: 0, fragment: 54816500 })
...
console.log('fetches', db.sqlite.pager.fetches, 'bytes', db.sqlite.pager.bytesFetched)
