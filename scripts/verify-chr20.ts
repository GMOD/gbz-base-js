...
for (const a of alignments) {
  byPath.set(
    a.pathHandle as number,
    (byPath.get(a.pathHandle as number) ?? 0) + 1,
  )
}
console.log('fragments', alignments.length, 'distinct paths', byPath.size)
for (const [handle, count] of [...byPath.entries()]
  .sort((x, y) => y[1] - x[1])
  .slice(0, 5)) {
  ...
  console.log(
    ' ',
    handle,
    `${p?.name.sample}#${p?.name.haplotype}#${p?.name.contig}`,
    count,
    'fragments',
  )
}
...
for (const a of sample) {
  ...
  const local = {
    start: (a.hapStart as number) - p.name.fragment,
    end: (a.hapEnd as number) - p.name.fragment,
  }
  const coordOk = (a.strand === '+' ? local.start : length - local.end) === bp
  const startOk =
    start.node === expectedStart.node && start.offset === expectedStart.offset
  if (coordOk && startOk) {
    ok += 1
  } else {
    bad += 1
    console.log('MISMATCH', a.pathHandle, a.strand, {
      start,
      expectedStart,
      bp,
      local,
